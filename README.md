...
- **マスク別パレット** … キャラ・背景などの**領域マスク**を読み込むと, **領域ごとに独立したパレット**（色数・しきい値・色OFF）で減色できます. マスクは**連番PNG（クリスタ等で作成）またはZIP**を直接読み込めます. 
...
マスクは「**動画と同じ縦横比の白黒画像**（**白＝対象・黒か透明＝背景**）を**1フレームにつき1枚**」用意するだけです：
