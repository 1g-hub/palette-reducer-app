# 🎨 Palette Reducer — 動画の色をかんたんに減らすアプリ

動画の色を、少数の「代表色」に置き換える（色数を減らす）ためのアプリです。
...
1. 「仕上がりの好み」を選びます。基本は **「おすすめ」** のままでOKです。
   ...
   - **高精細（遅い）**：きれいだけど時間がかかる
   - **軽量（速い）**：速いけど粗め
2. 「**○件の色を分析する**」を押すと分析が始まります。
3. 細かく調整したい人は「詳細設定（上級者向け）」を開くと、解像度や色数などを変えられます。
...
- **はみ出し色マップ**：🟣 **マゼンタ（鮮やかなピンク）= 代表色のどれにも当てはまらない「はみ出し色」** がどこにあるかを示します
...
1. 保存したい動画にチェックを入れます。
2. **保存形式**を選びます（ⓘ や説明文で各形式の特徴が分かります）。迷ったら **「WebM（標準・最速）」** が手軽です。
3. 「**⬇ 選択した○件を書き出す**」を押します。
...
4. 書き出し後も、**形式や対象を変えて「もう一度書き出す」**ことができます。最初からやり直すときは「最初に戻る」。
...
| 形式 | 特徴 | こんなときに |
|---|---|---|
| **WebM（標準・最速）** | ブラウザだけで高速に書き出し | まず手軽に保存したい |
