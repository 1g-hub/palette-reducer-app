...
## マスク移動 (M)：選択色マスクの平行移動＋十字キー微調整（2026-07-09, ユーザ要望）
- 要望「指定したマスクをそのまま平行移動。移動するマスクを選択し、十字キーで微調整」。新規 `move.js`（`?v=40`、scriptは snap.js の後）。
- 設計:
  - 対象＝**選択色・現フレーム**。ツール選択時に基準コピー(base)を取り、移動は常に「baseからの一括シフト」＝**端で往復しても画素が欠けない**（確定時に画面外分のみ切り捨て、ヘルプに明記）。純関数 `shiftLines`。
  - **ドラッグ＝大まか移動**（掴んでいる間は半透明ゴースト、離すと反映）／**矢印キー＝1px微調整（Shift=10px）**。矢印はツールが有効な間だけ **capture段リスナーで本体のフレーム移動より先に奪う**＝ツールを抜けると従来のフレーム移動が復活。
  - **Enter・ツール切替＝確定（diff 1件＝1 Undo）**／**Esc・未確定中のCtrl+Z＝取消**。確定まで undo 履歴を汚さず、未確定の生変更も `S.onFrameChanged` で自動保存に通知（保存と実データの不一致を防ぐ）。フレーム/選択色が変わっていたら旧移動を自動確定してから取り直し（mv.frame へのコミットなので移動後でも安全）。
- 検証: unit +6（shiftLines: 恒等・全画素移動・境界クリップ×3）→ **119/0**。**e2e `move-mask`（新規）16/0**: 移動結果が `shiftLines(base,dx,dy)` と**画素単位で完全一致**（+3,+2 / Shift粗動 −7,+2 / ドラッグ +15,+8）、矢印でフレームが動かない、Esc完全復元、Enter確定→**Ctrl+Z 1回で復元**、ツール切替の自動確定も1 Undo、**他の色レイヤは終始不変**、ツールを抜けたら矢印のフレーム移動が復活。回帰: smoke11/snap19/trace-snap9/trace-smooth14/frame-snap15/persist11 全て 0 FAIL。

## なぞり平滑 (Y)：なぞった区間を滑らかな1px線へ（2026-07-09, ユーザ要望）
- 要望「なぞったところをいい感じになめらかかつ、1ピクセルの線に変えたい」。**エッジ検出を一切使わない独立ツール**として追加（不採用となった内側吸着の教訓＝既存吸着挙動は不変更）。
- 実装（snap.js, `?v=39`※38は不採用実験がブラウザキャッシュに残りうるためスキップ）:
  - T の前段/後段を `pickSwathSection`（帯→最大線区間→自動アンカーA,B）/`replaceSection`（除去→ラスタ化→連鎖掃除→塗り3割崩壊ガード→1Undoコミット）へ**共通化**し、T は同挙動のまま（trace-snap 9/0・qa-sam-trace 15/0 で回帰確認）。
  - 新 `traceSmoothApply`: 区間内の既存線を `linePathWithin`（BFS）で**1本の順序付き糸**にたどり（局所的に2px厚の塊も1本に集約）、純関数 `smoothChain`（移動平均 窓7×なめらかさ回、**両端固定・端は点対称外挿**＝直線不変・端の接線保持）で平滑化→丸め→ラスタ化。
  - UI: 「〰 なぞり平滑 (Y)」ボタン＋「なめらかさ」スライダー(1..10, 既定4)。円カーソル/帯プレビューは水色（T=黄）。円の大きさ＝吸着半径スライダー共用。
- 検証:
  - unit +5（両端固定・ジグザグTV>75%減・直線不変・反復単調）→ **113/0**。
  - **e2e `trace-smooth`（新規）14/0**: ギザギザ±1px＋10列ごと2px厚の閉ループ上辺を実マウスで掃く→**TV 83→19（77%減）・2px厚 27→0列**・孤立0・ループ閉維持（fill 29974→29987）・他辺不変・1 Undo完全復元・スライダー/Yキー配線。
  - 実SAM QA `qa-sam-smooth`（新規, CL_SAM_DIR gated）**11/0**: 男の子頭頂部351px区間→339点、面積739938→739921、孤立増0、Undo復元、スクショ目視OK。**率直な注記: 実データのTVは134→130と小幅**＝頭頂部は「単調な階段」成分が主で、これは2値ラスタの原理上減らせない（斜め線は必ず階段）。効きどころは**振動ノイズ・手描きの震え・2px厚の塊**（合成テストの77%減が該当）。
  - 回帰: trace-snap 9/0（共通化後のT）・snap 19/0・frame-snap 15/0・smoke 11/0。

## 【不採用・全戻し】エッジ吸着「色遷移の内側」実験（2026-07-09, ユーザ要望→実使用で却下）
- 要望「大きく色が変わる所の**内側**へ・囲み未成立はカーソル側へ」を実装した: `outwardOffsets`（外向き方向場: 囲みあり=領域内外距離、開いた線=ストローク距離）+ `innerEdgeCost`（コストを自画素でなく外向き+1/+2の勾配で評価）→ 最小コストが遷移帯の中央（勾配の山）から**内側の縁**に移る。W/T/フレーム全体の3箇所へ接続、版38。
- 合成の定量e2e（ステップエッジでマスク503→501移動、Wはストローク側で反転、stash実証で旧コード2 FAIL）・実SAM QA 15/0・回帰全緑まで確認したが、**ユーザ実使用で「吸着してほしくない所に吸着し、とても使いづらい」→不採用**。
- **全戻し済み（2026-07-09）**: コミット前だったため（コミットは権限分類器の一時障害で保留のままだった）、`git checkout HEAD -- snap.js index.html test/test-contour.js` + inner-snap.js 削除で **HEAD 277b7cb（版37）と差分ゼロ**に復帰。検証: `git diff --stat HEAD` 空・unit **108/0**・`?v=37`×10。リポジトリには一切残っていない。
- 実験ファイル一式はスクラッチパッド `inner-snap-rejected/` に退避（snap.js/index.html/test-contour.js/inner-snap.js）。**教訓**: 合成エッジ＋実SAM静止QAでは「狙った境界に正しく寄る」ことしか測れず、「**狙っていない境界に引っ張られる**」使用感の劣化は検出できなかった。吸着の挙動変更は今後、必ず**オプション（トグル）として併存**させてユーザ比較を経てから既定化する。

## 細部（髪の先端）吸着精度の向上（2026-07-08/09, ユーザ要望「先が丸みを帯びる」）
- 現機構の説明（ユーザ質問への回答）: エッジ検出＝`colorSobelMag`（RGB各ch 3×3 Sobel の最大＝色境界も拾う、フレーム毎キャッシュ）。吸着＝アンカー間の回廊で `cost=1+8(1-勾配/max)+距離バイアス` の Dijkstra 最小コスト経路。
- **丸まりの構造的原因**: ①細いトゲは「入って戻る」経路が長く、根元の近道がコストで勝つ（細いトゲ内部はどこでも元輪郭に近いため距離バイアスが効かない） ②AAで先端の勾配が弱くエッジ項の引力が減る ③アンカーがトゲ間に落ちると経路が素通り可能。**再現**: 高さ12pxのトゲが frame-snap で10px削れる（227→237）ことをテスト先行で確認。
- **修正2点**: **(a) 鋭角アンカー**＝純関数 `sharpTurnIndices`（前後k=6点の弦の折れ角≥55°の局所最大）で輪郭のトゲ先端・角を検出し**固定アンカー**（勾配ナッジ無し）として追加→先端は必ず経路が通る。均等アンカーは鋭角の±12近傍では省略。**(b) ガンマ補正**＝`costFromMag` に gamma 引数（既定1=互換）を追加し W/T/フレーム全体の3箇所で **gamma=0.5**（弱いAAエッジの引力を増幅）。
- **結果**: トゲの丸まり **10px→0px**（227→227、frame-snap 15/0）。unit +4（sharpTurnIndices 先端検出・直線部で無反応、gamma 弱エッジのコスト低下・端点不変）→ **108/0**。実データQA 15/0＋スクショ目視で**左右のトゲに輪郭が巻き付く**改善を確認。W(snap19)/T(trace-snap9) 回帰なし。版37。
- 回帰: smoke11/carry-ghost14/cow15/merge-layers13/sam-import11、0 FAIL。

## フレーム全体吸着のドーナツ穴バグ修正（2026-07-07, ユーザ報告）
報告=「領域内に空間（穴）があると全部埋まる」。原因は2段:
- **(1) 穴の縁を処理していなかった**: 領域の旧線を全消し→外周だけ描き直すため、穴の縁の線が失われ even-odd で穴が充填。→ 領域bbox内で「この領域以外」をbbox縁から4近傍flood→届かない空間＝穴として検出し、**穴の縁も mooreBoundary→同じ吸着処理**（24px未満の小穴は形をそのまま維持）。穴の中の島（別領域）はblobに含めても外縁追跡に影響せず、島自身は別成分として処理される。
- **(2) 平坦部でループが退化**: 修正(1)後もテスト用の穴（暗い髪の内部＝エッジ信号なし）が埋まった。デバッグ描画で「縁は92px描かれているがループが潰れて囲めていない」ことを確認。原因＝回廊内で勾配を正規化するため平坦部でノイズ増幅＋最短経路化。→ コストを**グローバル正規化＋「元の輪郭からの距離」バイアス（回廊内BFS距離を加算）**に変更、アンカー移動も「グローバル最大の15%以上のエッジがあり今より良い時だけ」に。**エッジがある所は吸着し、無い所は元の形を保つ**＝ユーザ期待（Tと同じ効果）どおり。
- テスト駆動: frame-snap にドーナツを追加→現行コードで FAIL（穴が埋まる）を確認→修正→ **14/0**。実データQA（qa-sam-trace）**15/0**・面積+0.09%に改善、スクショ目視で**外周の忠実度も向上**（以前直線化していたトゲ付近もシルエット追従）。版36。
- 回帰: unit104/smoke11/snap19/trace-snap9/carry-ghost14/cow15/merge-layers13/sam-import11、0 FAIL。

## フレーム全体のエッジ吸着（2026-07-07, ユーザ要望）
「なぞり吸着を現フレームのエッジ全体に適用するボタン＋専用半径」を実装。
- `snapFrameEdges`（snap.js）: 選択色の**塗り領域（線∪fill）の8連結成分**ごとに、外周を**ムーア近傍追跡**（`mooreBoundary` 純関数）で順序付き閉輪郭化→約90px間隔のアンカー（局所勾配最大へ≤4px寄せ・隣接区間で共有＝閉のまま）→区間ごとに帯Dijkstraで置換。**領域ベースなのでギザギザ線のグラフの枝・行き止まりに頑健**（線グラフのウォークは実測11px/721pxで迷子→棄却）。塗りの無い開曲線・20px未満のノイズ片は対象外。内部の迷い線も領域ごと掃除。塗り3割崩壊で中止・1 Undo で全復元。UI=「🧲 フレーム全体のエッジへ吸着（選択色）」＋専用「全体半径」スライダー(2..16, 既定5)。
- **開発中に自分のバグを2つ検出→修正**: (1) 線グラフウォークがギザギザで迷子（→領域ベースに設計変更） (2) **ムーア追跡のJacob停止条件がギザギザで満たされず無限周回**（e2eタイムアウトで検出。→「(画素×走査方向)状態の再訪」停止に変更＝必ず停止。ユニットの穴（長さ>100だけ）も上限つきに強化）。
- unit +4（mooreBoundary: 四角リム24px一周・ギザギザで1周のみ・1px領域）→ **104/0**。**e2e `frame-snap`（新規）12/0**: 2輪郭を1発で両方吸着（トースト「2本」）・閉維持・ノイズ片(12px未満)不変・孤立増加なし・専用スライダーのラベル同期・1 Undo全復元。
- **実データQA**: qa-sam-trace にフェーズ2追加 → 実SAM男の子マスクで**5輪郭を一括吸着**・新規孤立0・面積+0.5%・1 Undo復元（**15/0**）。スクショ目視: 半径5では髪のトゲもほぼ保持し輪郭は連続・ゴミなし。版35。
- 回帰: smoke11/snap19/trace-snap9/frame-snap12/carry-ghost14/cow15/merge-layers13/sam-import11、0 FAIL。

## なぞり吸着を「円ブラシの帯」仕様に刷新（2026-07-07, ユーザ要望＋実データQA）
指摘＝「円は出るが結局1pxの軌跡しか吸着の要素にならない」。実際、旧設計は軌跡端点から別計算の許容半径（round(10/scale)）で線を拾っており、**円は飾りだった**。
- **新仕様**: 円（半径=吸着半径スライダー）が掃いた**帯(スワス)に触れた線の連結区間まるごと**が置換対象。区間の両端＝「線が帯の外へ出る画素（出口）∪ 区間内の線端点」の最遠ペアが自動アンカー。新しい線は帯の中の色エッジ最小コスト経路（Dijkstra、コスト正規化も帯内）。**線の真上を正確になぞる必要なし**。1点クリック＝円内だけの局所吸着。プレビューは帯そのもの（半透明の太帯＋中心線）を表示。
- 取り残し掃除を**連鎖式**に（削除した画素の隣も候補へ追加、ただし帯+3px内限定＝遠くの正規線・枝は不可侵）。
- e2e `trace-snap` を刷新: ズーム2倍・半径10で**軌跡を線から8pxずらして**掃く→旧コードは「なぞりの始点/終点を…」で拒否（stash実証 FAIL）、新コードは80px区間を置換・閉ループ維持・孤立0・Undo完全復元 → **9/0**。
- **実データQA（ユーザ依頼で実施, `qa-sam-trace` 新設・CL_SAM_DIRゲート）**: 実SAMの男の子マスク(fill 739,938px)を取込→頭頂部の輪郭351pxを円半径10で掃く→**333点の経路に置換・新規孤立0（既存1個はSAM由来のノイズ片と特定）・面積±0.005%・Undo復元** → **10/0**。before/afterスクショを目視確認: 輪郭は繋がったまま置換され、ゴミなし。**所見**: 帯にトゲ状ディテールを含めると最小コスト経路が根元をショートカットして単純化する（仕様上のトレードオフ。トゲ保持は円を小さく/避けて掃く）。
- 版34。回帰: unit 100/0、e2e smoke11/snap19/trace-snap9/qa-sam-trace10/carry-ghost14/cow15/merge-layers13/sam-import11/clear-range9/legacy-restore9/copy-forward12、0 FAIL。

## なぞり吸着の磨き込み3件（2026-07-07, ユーザ報告）
- **吸着半径スライダーの表示が動かない**: リスナー自体が無くラベルが「6」固定だった（値は使用時に読まれ内部的には効いていた）。→ input で ラベル＋`S.traceSnapR` を同期。snap e2e に回帰2件 → **19/0**。
- **Tのカーソルに半径円**: 消しゴム同様、なぞり吸着ツール時はカーソルに回廊半径（スライダー値×ズーム）の黄色い円を表示（drawCursor）。スライダーを動かすと円もライブで追従。
- **1px取り残しの解消**: 根因を実験で特定＝**実SAMマスクのギザギザ境界（1px凹凸）**では、旧区間除去（BFSの1本道のみ）で凹凸の角画素が孤立して残る（node実験: きれいな境界は0個・ギザギザ境界は8〜13個）。→ 置換後に「旧区間の8近傍にあった旧線画素」だけを候補として孤立/ぶら下がり(次数≤1)を反復掃除（新経路・アンカーは保護＝正規の線は削らない）。e2e `trace-snap` をギザギザ境界（決定的PRNG）に変更し「孤立0」を追加 → **9/0**、**旧コードで isolated=13 の FAIL を git stash 実証**。
- 版33。回帰: unit 100/0、e2e smoke11/snap19/trace-snap9/carry-ghost14/clear-range9/cow15/sam-import11/merge-layers13、0 FAIL。
